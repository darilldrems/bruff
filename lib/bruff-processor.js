...
class BruffProcessor {
    ...
    static _doTask(destination, context) {
        ...
        //check if destination.url is array, select one string then parse the selected string
        //this is a load balancing feature
        var destUrl = "";
        if (_.isArray(destination.url)) {
          destUrl = destination.url[Helpers.getRandomInt(0, destination.url.length - 1)];
        } else {
          destUrl = destination.url;
        }
        var url = ValueParser.parse(destUrl, context);

        var requestObject = {};
        requestObject.headers = context.client.req.headers;

        //response filters are method to call on
        //the response object
        var responseFilters = [];
        if (_.isArray(destination.after) && destination.after.length > 0) {
          responseFilters = destination.after;
        }
        ...
        new RequestBuilder(url, requestObject)
            .makeRequest(method)
            .then(function (resp) {
              //parse the body and if it fails because its not json
              //then just use the normal response as is.
              //this happens when upstream server returns 500, 404 or crashes
                try {
                  var jsonResp = JSON.parse(resp.body);
                  context.responses.push(jsonResp);
                  resp.body = jsonResp;
                  //once response is back and valid json
                  //run all response filters
                  _.each(responseFilters, function (filterFunction) {
                    filterFunction(resp);
                  });
